...
NGINX_BIN="${FNPROXY_NGINX_BIN:-$ROOT/third_party/nginx/x86_64/nginx}"

[[ -x "$NGINX_BIN" ]] || {
  echo "Missing x86_64 Nginx test binary: $NGINX_BIN" >&2
  echo "Run ./scripts/build-nginx.sh x86 first, or set FNPROXY_NGINX_BIN." >&2
  exit 1
}
...
cp "$NGINX_BIN" "$TEST/app/bin/nginx"
...
cleanup() {
  ...
  if [[ "$KEEP_TEST_ROOT" != "1" ]]; then rm -rf "$TEST"; else echo "Keeping test directory: $TEST"; fi
}
trap cleanup EXIT
